...
if redis_url:
    ...
    storage_options = {"storage_uri": redis_url}
else:
    logger.warning("Redis URL not found. Using in-memory storage for rate limiting (not recommended for production)")
...
def schedule_viewing():
    try:
        ...
        event = calendly_client.schedule_meeting(
            name="Guest User",  # You might want to get this from the session
            email=user_email,
        ...
        if event.get("status") == "success":
            ...
            return jsonify({"error": event.get("message", "Failed to create event")}), 500
            
    except Exception as e:
        logger.error(f"Error in schedule_viewing: {str(e)}")
        traceback.print_exc()
...
def get_available_times():
    try:
        ...
        available_times = calendly_client.get_available_slots()
        if available_times:
