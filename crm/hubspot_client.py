...
def validate_api_key():
    ...
    if not HUBSPOT_API_KEY:
        logger.warning("No HubSpot API key provided")
        return False
    ...
    try:
        ...
        logger.warning(f"HubSpot API key validation failed: {str(e)}")
        return False

# Validate API key on module load but don't raise error
if not validate_api_key():
    logger.warning("HubSpot integration will be disabled")
...
def create_or_update_contact(email, name, budget, lead_type, lead_score, qualification, chat_history, user_type):
    ...
    if not HUBSPOT_API_KEY:
        logger.warning("Skipping HubSpot update - no API key provided")
        return 200, {"status": "skipped", "message": "HubSpot integration disabled"}
