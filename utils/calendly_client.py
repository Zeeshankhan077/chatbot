...
class CalendlyClient:
    ...
    def schedule_meeting(self, name: str, email: str, start_time: str, event_type_uri: Optional[str] = None) -> Dict[str, str]:
        ...
        try:
            ...
            # Format the start time to ISO 8601
            start_time_dt = datetime.fromisoformat(start_time.replace('Z', '+00:00'))
            formatted_start_time = start_time_dt.strftime('%Y-%m-%dT%H:%M:%S%z')
            ...
            if e.response.status_code == 404:
                return {"status": "error", "message": "Event type not found. Please check your Calendly configuration."}
            return {"status": "error", "message": f"Calendly API error: {str(e)}"}
        except Exception as e:
