...
def handle_chat(name, email, message, chat_history, budget):
    ...
    lead_params = {
        "interest_level": min(20, num_messages * 3),  # Reduced from 30 to 20
        "budget_match": 15 if budget else 0,  # Reduced from 20 to 15
        "engagement_time": min(10, num_messages * 2),  # Reduced from 15 to 10
        "follow_up": 5 if "follow up" in message.lower() else 0,  # Reduced from 10 to 5
        "offer_response": 5 if "offer" in message.lower() else 0,  # Reduced from 10 to 5
        "appointment": 5 if "appointment" in message.lower() else 0,  # Reduced from 10 to 5
    ...
    answer, _, _, schedule_meeting, full_reply = call_groq_llama(context, message, lead_params)
